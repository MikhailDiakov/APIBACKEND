...
def save_log_to_cassandra(session, level, message, extra_data=None):
    log_id = uuid.uuid4()
    ...
    query = SimpleStatement(
        """
        INSERT INTO log_entries (id, level, message, extra_data, timestamp)
        VALUES (%s, %s, %s, %s, %s)
    """
    )

    session.execute(query, (log_id, level, message, extra_data_str, timestamp))
...
for message in consumer:
    ...
    save_log_to_cassandra(session, level, message_content, extra_data)
